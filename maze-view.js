...
export function mazeView(maze, player, rowInp, colInp){
    ...
    const currLevel = player.currLevel;
    mazeLevelHeader.textContent = `Level ${currLevel}`
        for (let j = 0; j < maze.rows; j++) {
            for (let k = 0; k < maze.cols; k++) {
                const cell = maze.maze[currLevel][j][k]
...
export function reRenderPlayerLocation(prevLocation, newLocation){
    const prevDiv = document.querySelector(`[data-id="${prevLocation}"]`);
    const nextDiv = document.querySelector(`[data-id="${newLocation}"]`);
